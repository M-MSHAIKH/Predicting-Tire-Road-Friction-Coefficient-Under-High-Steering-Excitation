...
%Training a LSTM
gru_net = trainnet(input_data_train,output_data_train,Networklayers,'l1loss',options);

%% Predictions

% Loading the previously trained LSTM
% load_gru = load("gru_22_03_01_02_PM.mat");
% gru_net = load_gru.gru_net;
%info = load_lstm.info;

% view(net_train);
y_norm = predict(gru_net,input_test);
% y_norm = y_norm';
...
time_plot = time_sec(num_valid+1:end,1);
time_plot = vertcat(time_plot,time_plot,time_plot,time_plot);
figure;
plot(y_output,'.');
hold on;
plot(output_test,'.')
title('\mu estimation using GRU')
% xlabel('Time(sec)')
ylabel('\mu estimated')
% xlim([10,40])
legend('NN outputs of \mu', 'True values of \mu')
grid on;
...
xlabel('Time (sec)')
% xlim([10,40])
legend('GRU prediction for wet surface','GRU prediction for ice sheet surface',...
    'GRU prediction for dry surface','GRU prediction for ice surface',...
    'True values for wet surface', 'True values for ice sheet surface',...
    'True values for dry surface','True values for ice surface')
ylim([0 1.2])
grid on;
%exportgraphics(h1,'gru_predictions.png','Resolution',300)
